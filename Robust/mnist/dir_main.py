...
dir = np.random.dirichlet((5,3,5), (28,28))
def add_noise(inputs,noise_factor=0.3):
  #print(inputs.shape)
  lst = [3] *28
  tup = tuple(lst)
  noisy = 0.2 * inputs + 0.8 * np.random.dirichlet(tup, (inputs.shape[0],1,28))
...
batch_size = 128
path = "dir_test_bs_"+str(batch_size)+"ls_"+str(n_z)+"ac_relu/"
os.mkdir(path)
    ...
    for model_num in range(total_models):
        ...
        for epoch in range(ITERS):
            ...
            for data in train_loader_subset_tar:
              ...
              #print(images.shape)
              x_latent = encoder(images_noisy.float()).to(device)
              ...
              mmd_loss = imq_kernel(x_latent.cuda(), z_fake.cuda(), h_dim=2)
